function [X_MC_graphs, stat_MC_graphs] = MC_function(Xn, Gc, Gr, ratio,...
    SeedVal, prob_params, solver_params)

% Keep 20% for training, the rest for validation
[y_train, mask_train, y_val, mask_val, y_test, mask_test] = split_observed(Xn, ratio, 0);
% y_val = paramData.y_val;
% mask_val = paramData.mask_val;
% y_train = paramData.y_train;
% mask_train  =paramData.mask_train;
% y_test = paramData.y_test;
% mask_test = paramData.mask_test;
% y_val = paramData.y_val;
% mask_val = paramData.mask_val;
...
% prob_params.Lc = (single(full(Gc.L)));
% prob_params.Lr = (single(full(Gr.L)));
...
% 
% 
% %% Solve with graphs, just Smoothness assumption
% prob_params.gamma_n = 0;
% prob_params.gamma_r = 0.05;
% prob_params.gamma_c = 0.05;
% solver_params.rho_ADMM = .3;
% 
% % Now ADMM is equivalent to forward backward algorithm!
% [X_MC_graphs_only, stat_MC_graphs_only] = MC_solve_ADMM(y_train, y_val, y_test, prob_params, solver_params);
% 
% 
% %% 
% figure;
...
% hold all
% plot(stat_MC_graphs_only.rmse_val);
% legend('low rank', 'low rank + graphs','only graphs')
